		...
		for (int i = start; i < end; i++) {
			...
			if (mMarkCount > 0 && i >= 0 && i < mMarkCount) {
				...
				if (mCenterIndex == i) {
					...
					if (!TextUtils.isEmpty(mAdditionCenterMark)) {
						...
						canvas.drawText(temp, 0, temp.length(), x - off, mTopSpace, mMarkTextPaint);
						mMarkTextPaint.setTextSize(mNormalTextSize);
			...
			for (int offset = -2; offset < 3; offset++) {
				...
				if (offset == 0) {
					// center mark
					mMarkPaint.setStrokeWidth(mCenterMarkWidth);
					canvas.drawLine(ox, mHeight - mBottomSpace, ox, mHeight - mBottomSpace + markHeight, mMarkPaint);
				} else {
					// other small mark
					mMarkPaint.setStrokeWidth(mMarkWidth);
					canvas.drawLine(ox, mHeight - mBottomSpace + smallMarkShrinkY, ox, mHeight - mBottomSpace + markHeight - smallMarkShrinkY, mMarkPaint);
				}
			}
			x += mIntervalDis;
		}
